...
import { Target, Flame, Minus, ArrowDown, X, Plus, GripVertical } from 'lucide-react'
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export function GoalModal({ open, onOpenChange, goal, selectedYear, onSave }: GoalModalProps) {
  ...
  const addSubtask = () => {
    ...
    if (!text) return
    setSubtasks(prev => [...prev, { id: crypto.randomUUID(), title: text, completed: false }])
    setNewSubtask('')
  }
      ...
      <DialogContent className="sm:max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2 text-xl">
            ...
            {goal ? 'แก้ไขเป้าหมาย' : `เป้าหมายใหม่สำหรับปี ${year}`}
          </DialogTitle>
          <DialogDescription>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5 mt-4 overflow-x-hidden">
          ...
          <div className="space-y-2">
            <label className="text-sm font-medium text-foreground">
              ...
              {subtasks.length > 0 && (
                <span className="text-xs text-muted-foreground ml-2">{subtasks.length} รายการ</span>
              )}
            </label>
            ...
            <AnimatePresence>
              {subtasks.map((st) => (
                ...
                >
                  <div className="flex-1 flex items-center gap-2 py-1.5 px-3 bg-muted/50 rounded-lg text-sm">
