...
export function SharePoster({ open, onOpenChange, goals, userName, userAvatar, year }: SharePosterProps) {
  ...
  const drawPoster = (canvas: HTMLCanvasElement, scale: number = 1, goalsToRender: Goal[], avatarImg?: HTMLImageElement | null) => {
    ...
    ctx.fillText(`เป้าหมายปี ${displayYear}`, w/2, 200*s)
    ...
    if (goalsToRender.length > 0) {
      ...
      ctx.fillText(`${displayYear} Vision Board • Made by Dome`, w/2, footerY + 100*s)
    } else {
      ...
      ctx.fillStyle = selectedTheme.accentColor
      ctx.font = `${22*s}px "IBM Plex Sans Thai", sans-serif`
      ctx.fillText(`${displayYear} Vision Board • Made by Dome`, w/2, h - 100*s)
    }
  }
  ...
  const downloadPoster = async () => {
    ...
    try {
      ...
      const fileName = `${displayYear}-goals-${userName}.png`
