...
// At runtime, missing env vars will cause API calls to fail → triggers offline mode
...
export const goalsApi = {
  async getAll(userId: string, year?: number): Promise<Goal[]> {
    console.log('goalsApi.getAll: Starting... year=', year)
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
      try {
        ...
        const fetchPromise = query.order('position', { ascending: true })
